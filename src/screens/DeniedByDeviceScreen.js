...
const DeniedByDeviceScreen = () => {
  ...
  const fetchDeniedCardholders = async () => {
    try {
      ...
      const response = await axios({
        method: "get",
        url: `${baseUrl}/api/GetDeniedDevices?Language=p&Tran_Type=S&Level1_ID=${selectedRegion}&Level2_ID=${selectedCity}&Level3_ID=${selectedLocation}&Level4_ID=${selectedArea}&Eqpt_Group_ID=${selectedBrand}&Start_Date=${startDate}&End_Date=${endDate}`,
        headers: headers,
      });
      ...
      if (response.status === 200 && response.data.Data) {
        ...
        if (response.data.Data.length > 0) {
          setSelectedPersonCardNo(response.data.Data[0].Eqpt_ID);
          setSelectedPersonName(response.data.Data[0].Eqpt_Title);
  ...
  const GetDeniedByHourCardholders = async () => {
    try {
      ...
      const response = await axios({
        method: "get",
        url: `${baseUrl}/api/GetDeniedByHour?Language=p&Level1_ID=${selectedRegion}&Level2_ID=${selectedCity}&Level3_ID=${selectedLocation}&Level4_ID=${selectedArea}&Eqpt_Group_ID=${selectedBrand}&Event_Date=${startDate}&FilterBy=Device&FilterValue=${selectedPersonCardNo}`,
        headers: headers,
      });
  ...
  const GetDeniedByDayCardholders = async () => {
    try {
      ...
      const response = await axios({
        method: "get",
        url: `${baseUrl}/api/GetDeniedByDay?Language=p&Level1_ID=${selectedRegion}&Level2_ID=${selectedCity}&Level3_ID=${selectedLocation}&Level4_ID=${selectedArea}&Eqpt_Group_ID=${selectedBrand}&Start_Date=${startDate}&End_Date=${endDate}&FilterBy=Device&FilterValue=${selectedPersonCardNo}`,
        headers: headers,
      });
  ...
  return (
    <View style={styles.container}>
        ...
        <ScrollView>
          {data.map((item, index) => (
            <TouchableOpacity
              ...
              onPress={() => handleRowClick(index, item.Eqpt_ID , item.Eqpt_Title)}
            >
      ...
      {/* <ScrollView> */}
      <View  style={styles.accessDeniedPerDayHeatMap} >
